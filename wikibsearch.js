      ...
      if (!err && bytesRead === 4) {
        ...
        fs.read(dump.files.to.fd, offset, 0, dump.sizeof_txt_told, indexR * dump.sizeof_txt_told, (err, bytesRead, data) => {
          if (!err && bytesRead === dump.sizeof_txt_told) {
            ...
            if (upper >= 65536) console.warn(`** title offset ${offset} (${parseInt(+upper, 16)} : ${parseInt(+lower, 16)}) is greater than 48-bits!`);
      ...
      if (!err && bytesRead === 4) {
        ...
        fs.read(dump.files.do.fd, record, 0, 12, indexR * 12, (err, bytesRead, data) => {
          if (!err && bytesRead === 12) {
            ...
            if (upper >= 65536) console.warn(`** dump offset ${offset} (${parseInt(upper, 16)} : ${parseInt(lower, 16)}) is greater than 48-bits!`);
  ...
  Object.entries(p).forEach(e => {
    ...
    switch (k) {
      case "0":
        if (v === 't-needed')
          o.needed = true;
        else if (['t', 't-', 't+', 't-needed', 't-check', 't+check'].indexOf(v) === -1)
